...
const convertToBase64 = (req, res, next) => {
  try {
    // Handle .fields() approach for owner photo
    if (req.files && req.files.photo && req.files.photo[0] && req.files.photo[0].buffer) {
      ...
      console.log(`📸 Converted owner photo to base64: ${file.originalname} (${file.size} bytes)`);
    ...
    if (req.file && req.file.buffer) {
      ...
      console.log(`📸 Converted single photo to base64: ${req.file.originalname}`);
    }
...
const uploadOwnerPhotoMulter = multer({
  ...
  return (req, res, next) => {
    uploadFunction(req, res, (err) => {
      ...
      if (req.files && req.files.photo && req.files.photo[0]) {
        console.log('✅ Owner photo uploaded to memory:', {
...
const uploadOwnerPhoto = [
  handleUploadError(uploadOwnerPhotoMulter),
  convertToBase64
];
